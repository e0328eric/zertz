...
const message = @import("./utility.zig").message;
...
pub fn UnionFind(comptime T: type) type {
    return struct {
        ...
        pub fn find(self: *Self, x: T) usize {
            const x_location = self.inner.get(x) orelse message("[Zertz Internal Error]: UnionFind.find");
        ...
        pub fn getComponentRepresentors(self: *Self) !ArrayList(T) {
            ...
            for (parent_data.items) |pd| {
                try output.append(
                    self.reverse.get(pd) orelse message(
                        "[Zertz Internal Error]: UnionFind.getComponentRepresentors",
                    ),
                );
            }
